...
def list_recent_jobs(dashboard_cfg: dict, report_def_id: str, since: datetime) -> list[dict]:
    """
    ...
    """
    url = f"{dashboard_cfg['url']}/api/reporting/jobs/list"
    resp = requests.get(
        url,
        params={"size": 50},
    ...
    for job in resp.json().get("data", []):
        ...
        # Match on report definition ID (field name varies slightly by version)
        job_def_id = (
        ...
        if isinstance(raw_ts, (int, float)):
            ...
            try:
                ...
                logging.warning(f"  Cannot parse timestamp '{raw_ts}' for job {job.get('_id')} — skipping.")
                continue
...
def download_job(dashboard_cfg: dict, job_id: str, output_path: Path) -> None:
...
def send_missing_alert(
    ...
    """Notify recipients that a scheduled report was NOT found."""
    now = datetime.now()
...
def check_scheduled_report(report: dict, cfg: dict, dry_run: bool = False) -> str:
    """
    Validate and deliver one scheduled report.

    Flow:
      1. Query job list for report_def_id within check_window_minutes
      2. Job found + completed → check delivery marker → download → email → write marker
      3. Job not found → send missing-report alert to recipients
    ...
    try:
        if dry_run:
            ...
            jobs = [{"job_id": "dry-run-job-000",
                     "completed_at": datetime.now(tz=timezone.utc),
                     "status": "completed"}]
        else:
            jobs = list_recent_jobs(dashboard_cfg, report["report_def_id"], since)
        ...
        if len(jobs) > 1:
            logging.info(f"[{rid}]   {len(jobs) - 1} additional job(s) in window — using most recent.")
        ...
        if not dry_run:
            ...
            download_job(dashboard_cfg, job["job_id"], output_path)
        else:
...
def main() -> None:
    ...
    for report in targets:
        results[check_scheduled_report(report, cfg, dry_run=args.dry_run)].append(report["id"])
